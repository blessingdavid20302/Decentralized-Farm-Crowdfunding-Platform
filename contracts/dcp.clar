...
(define-read-only (get-price-history (campaign-id uint) (target-block uint))
  (match (map-get? price-history { campaign-id: campaign-id, block-height: target-block })
    history (ok history)
    err-not-found
  )
)
...
(define-read-only (get-effective-funding-goal (campaign-id uint))
  (match (get-campaign-market-config campaign-id)
    config (calculate-dynamic-goal campaign-id)
    (ok u0) ;; default if no market config
  )
)
